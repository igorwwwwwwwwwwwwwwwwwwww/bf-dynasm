...
int main(int argc, char *argv[]) {
    ...
    // Start timing
    double total_start = timing_mode ? platform_posix_time_ms() : 0.0;
    double phase_start = total_start;

    size_t program_size;
    char *program = platform_posix_read_file(argv[arg_offset], &program_size);
    if (!program) {
        bf_error("Could not open file");
    }

    if (timing_mode) {
        double phase_end = platform_posix_time_ms();
        platform_posix_print_phase_time("File I/O", phase_start, phase_end);
    ...
    if (timing_mode) {
        double phase_end = platform_posix_time_ms();
        platform_posix_print_phase_time("Parsing", phase_start, phase_end);
        phase_start = phase_end;
    }
    ...
    if (optimize) {
        ...
        if (timing_mode) {
            double phase_end = platform_posix_time_ms();
            platform_posix_print_phase_time("AST Optimization", phase_start, phase_end);
            phase_start = phase_end;
        }
    }
    ...
    if (timing_mode) {
        double phase_end = platform_posix_time_ms();
        platform_posix_print_phase_time("JIT Compilation", phase_start, phase_end);
        phase_start = phase_end;
    }
    ...
    char *memory = platform_posix_allocate_guarded_memory(memory_size);
    if (!memory) {
    ...
    if (timing_mode) {
        double phase_end = platform_posix_time_ms();
        platform_posix_print_phase_time("Memory Allocation", phase_start, phase_end);
        phase_start = phase_end;
    }
    ...
    if (timing_mode) {
        double phase_end = platform_posix_time_ms();
        platform_posix_print_phase_time("Program Execution", phase_start, phase_end);
        phase_start = phase_end;
    }
    ...
    if (profile_mode) {
        ...
        if (!prof_out) {
            ...
            free(program);
            platform_posix_free_guarded_memory(memory, memory_size);
            if (ast) ast_free(ast);
            return 1;
        }
    ...
    platform_posix_free_guarded_memory(memory, memory_size);
    if (timing_mode) {
        double total_end = platform_posix_time_ms();
        fprintf(stderr, "%-20s  --------\n", "");
        platform_posix_print_phase_time("Total Time", total_start, total_end);
    }
